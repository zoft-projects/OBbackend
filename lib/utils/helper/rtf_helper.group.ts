// This is based on rtf parser library
const rtfToTxt = (rtf: string): string => {
  try {
    if (!rtf.includes('rtf1') || !rtf.includes('\\')) {
      return rtf;
    }
    ...
    let match: string[], word: string, arg: string, hex: string, char: string, brace: string, tchar: string;
